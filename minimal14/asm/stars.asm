...
MakeStars:      MIV 0x00f0,z0                                    ; make N stars
  ...
  makestar:     JPS _Random ANI 3 SDT ptr INV ptr                ; quadrant of star (0, 1, 2, 3)
  redox:        JPS _Random CPI 200 FCS redox SDT ptr INV ptr    ; unsigned x
  redoy:        JPS _Random ANI 0x7f CPI 120 FCS redoy SDT ptr INV ptr ; unsigned y
  redoz:        JPS _Random SDT ptr INV ptr                      ; unsigned z
...
Div256x8_8:     CLZ inta+1 CLV intc                              ; set A MSB = 0, result C = 0 plus endmarker
  ...
  ds_entry:     LLV inta FCS ds_always FEQ ds_never              ; shift up A into empty MSB, watch out for carry
                  SUZ intb FCC ds_never                          ; CASE C=0: A >= B?
                    SDZ inta+1 INZ intc+0                        ; subtract B from A_MSB, set bit 0 of C
...
UpdateStars:    MIV STARDATA,ptr
  updateloop:   LDT ptr CPI 0xff FNE nextstar
                  RTS
  nextstar:     SDZ z0                                           ; quad -> z0
                AIV 4,ptr MTZ ptr,xa                             ; old screen position to (xa,ya) for deletion
                INV ptr MTZ ptr,ya
                SIV 4,ptr MTZ ptr,inta+0                         ; x -> inta
    ...
    youtside:     DEV ptr                                        ; move down from y -> x
                  JPS DeleteStar
    redox2:       JPS _Random CPI 200 FCS redox2 SDT ptr INV ptr ; new unsigned x
    redoy2:       JPS _Random ANI 0x7f CPI 120 FCS redoy2 SDT ptr ; new unsigned y
    unchanged:    AIV 4,ptr FPA updateloop                       ; goto start of next star element

    zinside:    SDZ intb                                         ; z -> intb
                JPS Div256x8_8                                   ; nsx = 256 * x / z
                CIZ 0,intc+1 FNE xoutside                        ; nsx outside viewport?
                CIZ 200,intc+0 FCS xoutside
                  MZZ intc+0,z1                                  ; nsx -> z1
                  DEV ptr MTZ ptr,inta                           ; abs(y) -> inta, z in intb remains unchanged
  ...
  changed:            AIV 2,ptr                                  ; goto sx
                      MZT z1,ptr INV ptr                         ; update screen coordinates (sx, sy)
                      MZT intc+0,ptr INV ptr                     ; ptr now points to next element
