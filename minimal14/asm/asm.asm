...
; CHANGE LOG
; 09.02.2025: Improved handling of 'fast jump' argument (type 4) in pass 2 (allowing for FPA 0xcc, see asm.h).
; 09.02.2025: Implemented error check at the end of the document for unprocessed arguments.
; 12.02.2025: Conversion to instruction set 1.4
...
#page
  ...
  p1_while:   JPS findelem                                    ; sets ep, elen and enext
              CPI -1 BEQ Error                                ; Error Invalid element (e.g. open string "...)
                CPI 0 BNE p1_nextel
                  CIZ 0,args BNE Error                        ; check for unprocessed arguments
                    JPA PassSymbols

  p1_nextel:    MVV enext,eptr DEV eptr                       ; TEST FOR LABEL DEF, goto last char of element
  ...
  p1_parse:     LDZ args ANI 0x0f                             ; PARSE ARG-SPECIFICALLY, use lower nibble
    ...
    p1_arg3:      JPS parseexpr                               ; *** EXPECT WORD ***
      ...
      p1_byte3:   LDZ args ANI 0xf0 ORI 0x01 SDZ args         ; change expectation to byte
                  INV pc JPA p1_next
    ...
    p1_arg1:      JPS parseexpr                               ; *** EXPECT BYTE ***
      ...
      p1_nextarg: LDZ args RL5 ANI 0x0f SDZ args              ; args >>= 4
                  JPA p1_next
    ...
    p1_arg0:      LDT ep CPI "'" BEQ p1_string                ; *** EXPECT ANYTHING ***
      ...
      p1_else:      JPS parseexpr                             ; call with isparse=0, sets: isop, islsb, ismsb, isword
                    CIZ 1,isop FNE p1_notop
                      RZP expr,>arguments,1 SDZ args          ; access FLASH argument data on bank 1
...
PassTwo:      ; JPS _Print "Pass 2", 10, 0
  ...
  p2_while:   JPS findelem                                    ; sets ep, elen and enext if successful (A=1)
              CPI -1 BEQ Error                                ; Error Invalid element (e.g. open string "...)
              CPI 0 BNE p2_nextel
                CIZ 0,args BNE Error                          ; check for unprocessed arguments
                  JPA PassDone

  p2_nextel:  MVV enext,eptr DEV eptr                         ; look at last char of element for ':'
                ...
                CIT '#',ep BNE p2_parse
                  ...
                  CIZ 5,elen BNE p2_testlen4
                    ...
                    INV eptr CIT 'e',eptr BNE Error
                      LDZ pc+0 NEG SDZ Z0 ADV pc              ; int delta = (-(pc & 0xff)) & 0xff; pc += delta;
  ...
  p2_parse:     LDZ args ANI 0x0f                             ; PARSE ARG-SPECIFICALLY, use lower nibble
    ...
    p2_arg4:      JPS parseexpr                               ; *** EXPECT FAST JUMP ***
                  ...
                  CZZ expr+1,pc+1 BEQ p2_arg4ok               ; matching MSBs?
                  CIZ 0,expr+1 BNE Error                      ; MSB != 0
      p2_arg4ok:    INV pc
                    CIZ 1,isemit BNE p2_nextarg
                      MZT expr+0,mc INV mc                    ; emit LSB
                      JPA p2_nextarg
    ...
    p2_arg3:      JPS parseexpr                               ; *** EXPECT WORD ***
      ...
      p2_reuse2:    INV pc
                    CIZ 1,isemit FNE p2_reuse
                      MZT expr+0,mc INV mc                    ; emit LSB
      p2_reuse:     LDZ args ANI 0xf0 ORI 0x01 SDZ args       ; args = (args & 0xf0) | 0x01
                    JPA p2_next
      p2_arg3m:   CIZ 1,ismsb FNE p2_arg3w
                    INV pc
                    CIZ 1,isemit FNE p2_reuse
                      MZT expr+1,mc INV mc                    ; emit MSB
                      FPA p2_reuse
    ...
    p2_arg0:      CIZ 3,elen BLE p2_arg0ex                    ; *** EXPECT ANYTHING ***
                  CIT "'",ep FEQ p2_arg0qu
                  CIT '"',ep BNE p2_arg0ex                    ; treat as expression
      p2_arg0qu:    INV ep SIZ 2,elen                         ; consume leading "
                    AZV elen,pc                               ; pc += elen-2
        p2_arg0nx:  MTT ep,mc
                    INV ep INV mc DEZ elen BGT p2_arg0nx      ; copy the string
                      JPA p2_next
      ...
      p2_arg0l:   CIZ 1,islsb FEQ p2_arg0ok                   ; goto emit LSB without arg-shifting
                  CIZ 1,ismsb FNE p2_arg0w
                    INV pc
                    CIZ 1,isemit BNE p2_next
                      MZT expr+1,mc INV mc                    ; emit MSB without arg-shifting
                      JPA p2_next
      p2_arg0w:   CIZ 1,isword FNE p2_arg0e
                    AIV 2,pc
                    CIZ 1,isemit BNE p2_next
                      MZT expr+0,mc INV mc                    ; emit LSB without arg-shifting
                      MZT expr+1,mc INV mc                    ; emit MSB without arg-shifting
                      JPA p2_next
  ...
  p2_nextarg:   LDZ args RL5 ANI 0x0f SDZ args                ; args >>= 4 (consume argument, advance pipeline)
                JPA p2_next
...
parseexpr:      CLZ isop CLZ isword CLZ islsb CLZ ismsb       ; reset parse flags
                ...
                CPI 0xff FEQ pe_notop
                  SDZ expr+0 MIZ 1,isop RTS                   ; SUCCESS
  ...
  pe_testchar:  LDT eptr CPI '"' BEQ pe_isquote               ; parse 'A'
                  CPI "'" BNE pe_testhex
    pe_isquote: SDZ Z0                                        ; store quotation start marker
                CIZ 3,elen BCC Error                          ; Error '...' in expr
                  INV eptr MTZ eptr,term                      ; store the char as lsb of term
  ...
  pe_dolabel:   MVV eptr,Z0                                   ; LABEL OR EMBEDDED MNEMONIC?
    ...
    frl_exit:   SVV eptr,Z0                                   ; Z0..1 = length of label/mnemonic
                CIZ 0,Z0 BEQ Error                            ; = LDZ Z0 CPI 0, Error Empty expression
                  JAS OpCode CPI 0xff FEQ pe_tstlabel         ; call expects A = length
                    SDZ term FPA pe_consume
  pe_tstlabel:    MIZ 1,isword
                  CIZ 0,isparse FEQ pe_consume
                    JPS findlabel CPI 0 BEQ Error             ; Error Unknown reference
                      MTZ lptr,term+0 DEV lptr                ; extract term from PC value in label database
                      MTZ lptr,term+1
...
; modifies: 0x80..0x8a
; ----------------------------------------------------------
OpCode:         CPI 3 BEQ oc_normal                           ; test for A = length 3 (ABC) or 4 (AB.C)
...
putlabel:       MVV ep,eptr LDT eptr CPI ':' FEQ pl_exit      ; do not store empty labels
  pl_copy:        SDT lptr INV eptr DEV lptr                  ; store label including : backwards
                  LDT eptr CPI ':' FNE pl_copy
                             SDT lptr DEV lptr                ; store ':' end marker and move down
                    MZT pc+0,lptr DEV lptr
                    MZT pc+1,lptr DEV lptr
...
linenr:         MIW 0x0001,ln_num
  ...
  ln_loop:      LDZ ep+0 CPB ln_ptr+0 BNE ln_noteq
                  LDZ ep+1 CPB ln_ptr+1 BNE ln_noteq
...
; -------------------------------------------------------------------------------------------------------------
findelem:         CIT 0,ep FEQ fe_eofelem                     ; = LDT ep CPI 0, check for EOF
  ...
  fe_findlength:  MVV ep,elen                                 ; START ELEMENT LENGTH CALCULATION
    ...
    fe_consume:       INV elen                                ; ... while element goes on...
      ...
      fe_retlength:         MVV elen,enext SVV ep,elen        ; return valid elen, enext
                            LDI 1 RTS
    fe_eofelem:     CLV elen LDI 0 RTS                        ; return "EOF element"
    fe_error:       CLV elen LDI -1 RTS                       ; return "invalid element length"
  fe_quote:       SDZ Z0                                      ; ENTERING QUOTE, store quotation style
