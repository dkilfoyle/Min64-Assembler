...
#org 0x8000     MIB 0xfe,0xffff
                CLW score CLB gamestate

  mainloop:     CLW counter
    waitloop:   JPS KeyHandler
                INW counter CPI 0x06 BCC waitloop             ; adjust game speed to your liking
                  INB framecount
                  LDB gamestate
                  DEC BCC gamestate0
                    DEC BCC gamestate1
  ...
  newhigh:          LDB score+1 SDB hiscore+1
                    LDB score+0 SDB hiscore+0
                    LDI 22 SDB _XPos LDI 2 SDB _YPos LDB hiscore+0 PHS LDB hiscore+1 PHS JPS DecPrint PLS PLS
  nonewhigh:    LDI 2 SDB _XPos LDI 27 SDB _YPos JPS _Print 'PRESS <SPACE>', 0
                LDI 6 SDB _XPos LDI 7 SDB _YPos JPS _Print '   GAME OVER   ', 0
...
gamestate2:     LDB a_total CPI 0 BNE aliensleft              ; check if some aliens are left over
  ...
  checkfire:    LDB fire CPI 1 BNE mainloop
                  ...
                  ; SETUP NEW LEVEL -----------------------------
gamestate1:       LDI <ViewPort+0x0600 SDB vc_loopx+1         ; CLEAR GAME AREA
                  LDI >ViewPort+0x0600 SDB vc_loopx+2
    ...
    vc_loopx:     CLW 0xcccc
                  ...
                  LDI <0x788c SDB vl_loopx+3                  ; DRAW BOTTOM LINE
                  LDI >0x788c SDB vl_loopx+4
                  MIZ 28,0
    vl_loopx:     LDI 0xff SDB 0xcccc INW vl_loopx+3
                  ...
                  LDI 13 SDB _XPos LDI 2 SDB _YPos            ; print level
...
gamestate0:     JPS _Clear
                LDI 2 SDB _XPos CLZ _YPos JPS _Print 'SCORE<1>', 0
                LDI 18 SDB _XPos CLZ _YPos JPS _Print 'HI-SCORE', 0
                LDI 12 SDB _XPos LDI 5 SDB _YPos JPS _Print 'PLAY', 0
                LDI 7 SDB _XPos LDI 8 SDB _YPos JPS _Print 'ALIEN INVADERS', 0
                LDI 3 SDB _XPos LDI 13 SDB _YPos JPS _Print '* SCORE ADVANCE TABLE *', 0
                LDI 10 SDB _XPos LDI 16 SDB _YPos JPS _Print '= MYSTERY', 0
                LDI 10 SDB _XPos LDI 18 SDB _YPos JPS _Print '= 30 POINTS', 0
                LDI 10 SDB _XPos LDI 20 SDB _YPos JPS _Print '= 20 POINTS', 0
                LDI 10 SDB _XPos LDI 22 SDB _YPos JPS _Print '= 10 POINTS', 0
                LDI 2 SDB _XPos LDI 27 SDB _YPos JPS _Print 'PRESS <SPACE>', 0
                LDI 17 SDB _XPos LDI 27 SDB _YPos JPS _Print 'CREDIT 00', 0
                LDI 3 SDB _XPos LDI 25 SDB _YPos JPS _Print '<A> -- <D>', 0
                ...
                LDI 2 SDB _XPos LDI 2 SDB _YPos LDB score+0 PHS LDB score+1 PHS JPS DecPrint PLS PLS
                LDI 22 SDB _XPos LDI 2 SDB _YPos LDB hiscore+0 PHS LDB hiscore+1 PHS JPS DecPrint PLS PLS
  waitstart:    JPS _Random JPS _WaitInput CPI ' ' BNE waitstart
                  ...
                  LDI 16 SDB shippos
                  LDI 2 SDB _XPos LDI 2 SDB _YPos LDB score+0 PHS LDB score+1 PHS JPS DecPrint PLS PLS
                  LDI 12 SDB _XPos CLZ _YPos JPS _Print 'WAVE', 0
                  LDI 2 SDB _XPos LDI 27 SDB _YPos JPS _Print '3            ', 0 ; number of lives
                  ...
                  INB gamestate                               ; goto level setup
                  JPA mainloop

; ----------------------------------------------------------------------------------------------

ResetSaucer:      CLB u_state
                  LDI 0x58 SDB u_timer+0
                  LDI 0x02 SDB u_timer+1
                  RTS
...
UpdateSaucer:     LDB u_state
  ...
  sr_state4:      DEW u_timer BCS sr_rts
                  LDB u_pos RL6 ANI 31 SDB _XPos LDI 4 SDB _YPos
  ...
  sr_state3:      DEW u_timer BCS sr_rts
                    ...
                    LDB u_valptr+0 CPI <u_values+9 BCS ResetSaucer ; too many tries -> no score adv and return from there
                      SUI <u_values LL2 SDB u_timer+0
                      LDI >u_text SDB u_timer+1
  ...
  sr_state0:      DEW u_timer BCS sr_rts
                    LDB a_total CPI 12 BCC sr_rts
                      JPS _Random ANI 2 SUI 1 SDB u_step
                      CPI 1 BNE sr_notone
                        SDB u_pos JPA sr_setval
    sr_notone:        LDI 208 SDB u_pos
    sr_setval:        LDI <u_values SDB u_valptr+0            ; point to 300 points
                      LDI >u_values SDB u_valptr+1
                      INB u_state
    sr_rts:       RTS
...
as0_reset:        LDB as0_active CPI 0xff BEQ as_rts
                    LDI 0xff SDB as0_active                   ; deactivate this shot
...
as1_reset:        LDB as1_active CPI 0xff BEQ as_rts
                    LDI 0xff SDB as1_active                   ; deactivate this shot
...
as2_reset:        LDB as2_active CPI 0xff BEQ as_rts
                    LDI 0xff SDB as2_active                   ; deactivate this shot
...
as_manage:        LDB level LL2 SDZ 0                         ; time to place a new shot?
  ...
  as_redorand:      JPS _Random RL5 ANI 15 CPI 11 BCS as_redorand ; pick a random column 0..10
                  ...
                  JPS _Random CPZ 0 BCS as_usecol             ; time for a precise shot?
                    ...
                    CPI 176 BCS as_usecol
                      RL5 ANI 15 SDB as_col                   ; pick the column under which the player ship is located

  as_usecol:        CLB as_c
    as_cloop:       CLB as_r
    as_rloop:       LDB as_col ADB as_c CPI 11 BCC as_cokay
                      SUI 11
      as_cokay:     SDB as_ptr+0 SDB as_colcmod               ; (col+c) % 11, store result for later
                    LDI >a_alive SDB as_ptr+1
    ...
    as_living:        LDB as_r CPI 5 BCS as_trynext           ; no alien found in this column
                        LDB as_colcmod LL4 ADI 8 ADB a_x SDB as_px ; put exactly below alien (upper row of shot pixels is empty)
                        LDB as_r LL4 NEG ADB a_y SDB as_py
                        LDB as0_active CPI 0xff BNE as_try1   ; find a free slot
                          LDB as_px SDB as0_x LDB as_py SDB as0_y
                          LDI 0xff SDB as0_timer LDI 1 SDB as0_active
                          RTS
    as_try1:            LDB as1_active CPI 0xff BNE as_try2
                          LDB as_px SDB as1_x LDB as_py SDB as1_y
                          LDI 0xff SDB as1_timer LDI 1 SDB as1_active
                          RTS
    as_try2:            LDB as2_active CPI 0xff BNE as_rts
                          LDB as_px SDB as2_x LDB as_py SDB as2_y
                          LDI 0xff SDB as2_timer LDI 1 SDB as2_active
                          RTS
...
as0_update:        LDB as0_active CPI 0xff BEQ as_rts         ; is this slot active?
                    LDB as0_timer CPI 0xff BNE as0_explosion  ; shot is exploding
                      INB as0_y CPI 207 BCC as0_falling       ; the shot is currently falling down
                        ...
                        LDI 207 SDB as0_y PHS
                        JPS DrawSmall PLS PLS PLS
                        LDI 12 SDB as0_timer
                        RTS
  as0_falling:        ; shot is still falling down -> COLLISION DETECTION
                      LDB as0_y ADI 7 LL6 SDB addr+0          ; LSB of ypos*64
                      LDB as0_y ADI 7 RL7 ANI 63 ADI >ViewPort SDB addr+1 ; MSB of ypos*64 (rotate via C)
                      LDB as0_x RL6 ANI 63 ADI <ViewPort OR.B addr+0 ; xpos/8
                      LDB as0_x ANI 7 ADI LL0+0 SDB as0_llx   ; use sub pixel pos
                      LDI 1
                      ...
                      ANR addr CPI 0 BEQ as0_emptyspace
                        ...
                        LDB as0_y CPI 184 BCC as0_anywhite    ; was the player's ship hit?
                          CPI 192 BCS as0_anywhite
                            LDI 60 SDB waitframes
                            LDI 3 SDB gamestate               ; ship destroyed
                            RTS
...
as1_update:       LDB as1_active CPI 0xff BEQ as_rts          ; is this slot active?
                    LDB as1_timer CPI 0xff BNE as1_explosion  ; shot is exploding
                      INB as1_y CPI 207 BCC as1_falling       ; the shot is currently falling down
                        ...
                        LDI 207 SDB as1_y PHS
                        JPS DrawSmall PLS PLS PLS
                        LDI 12 SDB as1_timer
                        RTS
  as1_falling:        ; shot is still falling down -> COLLISION DETECTION
                      LDB as1_y ADI 7 LL6 SDB addr+0          ; LSB of ypos*64
                      LDB as1_y ADI 7 RL7 ANI 63 ADI >ViewPort SDB addr+1 ; MSB of ypos*64 (rotate via C)
                      LDB as1_x RL6 ANI 63 ADI <ViewPort OR.B addr+0 ; xpos/8
                      LDB as1_x ANI 7 ADI LL0+0 SDB as1_llx   ; use sub pixel pos
...
as2_update:       LDB as2_active CPI 0xff BEQ as_rts          ; is this slot active?
                    LDB as2_timer CPI 0xff BNE as2_explosion  ; shot is exploding
                      INB as2_y CPI 207 BCC as2_falling       ; the shot is currently falling down
                        ...
                        LDI 207 SDB as2_y PHS
                        JPS DrawSmall PLS PLS PLS
                        LDI 12 SDB as2_timer
                        RTS
  as2_falling:        ; shot is still falling down -> COLLISION DETECTION
                      LDB as2_y ADI 7 LL6 SDB addr+0          ; LSB of ypos*64
                      LDB as2_y ADI 7 RL7 ANI 63 ADI >ViewPort SDB addr+1 ; MSB of ypos*64 (rotate via C)
                      LDB as2_x RL6 ANI 63 ADI <ViewPort OR.B addr+0 ; xpos/8
                      LDB as2_x ANI 7 ADI LL0+0 SDB as2_llx   ; use sub pixel pos
                      LDI 1                                   ; generate pixel mask
...
UpdateShot:       LDB s_state DEC BCC us_rts
  ...
  us_fired:       JPS DeleteLaser                             ; delete laser at the old poition
                  LDB s_y CPI 28 BGT us_flying                ; new position would be 4 pixels higher (24)
                    ...
                    LDI 24 SDB s_y PHS
                    JPS DrawSmall PLS PLS PLS
                    LDI 2 SDB s_state LDI 12 SDB s_timer
                    RTS
    us_flying:    MIZ 4,1                                     ; PIXEL-EXACT COLLISION DETECTION starting from old top
                  LDB s_y LL6 SDB addr+0                      ; LSB of ypos*64
                  LDB s_y RL7 ANI 63 ADI >ViewPort SDB addr+1 ; MSB of ypos*64 (rotate via C)
                  LDB s_x RL6 ANI 63 ADI <ViewPort OR.B addr+0 ; xpos/8
                  LDB s_x ANI 7 ADI LL0+0 SDB us_llx          ; use sub pixel pos
                  LDI 1                                       ; generate pixel mask
  ...
  us_flyloop:     DEB s_y LDI 64 SUW addr LDR addr            ; move one pixel up
                  ANB us_mask CPI 0 BEQ us_nopixel
                    ...
                    LDB s_x SUB a_x+0 SDB s_dx
                      RL5 ANI 15 SDB s_ix                     ; horizontal tile index of current alien
                    LDB a_y SUB s_y SDB s_dy
                      RL6 ANI 31 SDB s_by LR1 SDB s_iy        ; vertical byte index, vertical tile index
                    LDB s_dx CPI 176 BCS us_noalien
                      LDB s_dy CPI 80 BCS us_noalien
                        LDI >a_alive SDB us_ptr+1
                        LDB s_iy SDB us_ptr+0
                        ...
                        LDR us_ptr CPI 1 BNE us_noalien
                          ; alien getroffen
                          LDI 0 SDR us_ptr                    ; mark alien as dead
                          DEB a_total LDI 1 SDB a_halt
                          LDI 12 SDB s_timer LDI 3 SDB s_state
                          LDI 11 PHS
                          LDB s_ix LL4 ADB a_x+0 SDB s_x PHS
                          LDB s_by LL3 NEG ADB a_y SUI 7 SDB s_y PHS
...
ResetAliens:      LDI 26 SDB a_x+0 CLB a_x+1
                  LDB level ADI 15 LL3 DEC SDB a_y
                  LDI 0xff SDB a_num
                  LDI 55 SDB a_total
                  LDI 2 SDB a_step
                  CLB a_halt
                  CLB a_costume
                  MIZ 55,0
                  LDI <a_alive SDB ra_loop+3
                  LDI >a_alive SDB ra_loop+4
  ra_loop:        LDI 1 SDB 0xffff INW ra_loop+3 DEZ 0 BGT ra_loop
                  RTS

UpdateAliens:     LDB a_halt CPI 1 BEQ ua_rts
  ua_nextnum:       INB a_num CPI 55 BCC ua_below
                      CLB a_num LDB a_costume XRI 1 SDB a_costume ; change costume
                      JPS ua_movexstep
  ua_below:         LDI <a_alive SDB ua_ptr+1
                    LDI >a_alive SDB ua_ptr+2
  ...
  ua_ptr:           LDB 0xffff                                ; self-modifying code here
                    CPI 0 BEQ ua_nextnum
                      ...
                      SDB a_pic LL4 SDB a_r
                      PLS LL4 SDB a_c
                      LDB a_pic ANI 6 ORB a_costume           ; prepare costume
                      PHS
                      LDB a_x+0 SDB ua_pos+0                  ; prepare x
                      LDB a_x+1 SDB ua_pos+1
                      ...
                      LDB a_y SUI 7 SUB a_r CPI 192 BCC ua_notlanded ; check for aliens reaching the ground
                        LDI 1 SDB lives LDI 60 SDB waitframes ; take away spare lives...
                        LDI 3 SDB gamestate                   ; ... and destroy ship
                        RTS
...
KeyHandler:     INK CPI 0xff BEQ key_rts
  ...
  key_entry:    CPI 0x29 BEQ isspace
                ...
                CPI 0x23 BEQ isd
  key_rts:        RTS
isa:            MBB pressed,left ORI 1 SDB pressed CLB right RTS
isspace:        MBB pressed,fire ORI 1 SDB pressed RTS        ; no counterpart to clear here
isd:            MBB pressed,right ORI 1 SDB pressed CLB left RTS
...
release:        CLZ 0xff                                      ; use 0xff as a byte counter
  ...
  key_release:  SDZ 0xff CLB pressed LDZ 0xff FPA key_entry   ; released key was received! -> analyze it now
...
; ----------------------------------------------------------------------------------
DrawSprite:     CLW mask+1 LDI 0xff SDB mask+0 SDB mask+3     ; prepare the erase rect mask
                LDI <sprites SDB dptr+0 LDI >sprites SDB dptr+1 ; init sprite data pointer
                LDS 5 LL4 ADW dptr                            ; point to sprite num (+ num*16 bytes)
                LDS 3 LL6 SDB addr+0                          ; LSB of ypos*64
                LDS 3 RL7 ANI 63 ADI >ViewPort SDB addr+1     ; MSB of ypos*64 (rotate via C)
                LDS 4 RL6 ANI 63 ADI <ViewPort OR.B addr+0    ; xpos/8
                LDS 4 ANI 7 SDZ 2 DEC FCC maskdone            ; store sub byte pixel pos
                  SDZ 0
  maskloop:       LLL mask DEZ 0 FCS maskloop                 ; shift mask once to pixel position
  maskdone:     MIZ 8,1                                       ; number of lines to process
  lineloop:     LDR dptr SDB buffer+0 INW dptr                ; copy the sprite bit mask
                LDR dptr SDB buffer+1 INW dptr CLB buffer+2
                LDZ 2 DEC FCC shiftdone                       ; shift that buffer to pixel position
                  SDZ 0
  shiftloop:      LLL buffer DEZ 0 FCS shiftloop
  shiftdone:    LDB mask+1 ANR addr ORB buffer+0 SDR addr INW addr ; delete old sprite rect
                LDB mask+2 ANR addr ORB buffer+1 SDR addr INW addr ; and draw new sprite
                LDB mask+3 ANR addr ORB buffer+2 SDR addr LDI 62 ADW addr ; ... and move to the next line
...
; modifies: X, Y registers
; ----------------------------------------------------------------------------------
DrawShot:       LDI <shots SDB dptr+0 LDI >shots SDB dptr+1   ; init sprite data pointer
                LDS 5 LL3 ADW dptr                            ; point to shot num (+ num*8 bytes)
                LDS 3 LL6 SDB addr+0                          ; LSB of ypos*64
                LDS 3 RL7 ANI 63 ADI >ViewPort SDB addr+1     ; MSB of ypos*64 (rotate via C)
                LDS 4 RL6 ANI 63 ADI <ViewPort OR.B addr+0    ; xpos/8
                LDS 4 ANI 7 SDB shift                         ; store sub byte pixel pos
                MIZ 8,1                                       ; number of lines to process
  ds_lineloop:  LDR dptr SDB buffer+0
                ORI 0xf8 SDB buffer+2 INW dptr                ; prepare the bit masks
                CLB buffer+1 LDI 0xff SDB buffer+3
                MBZ shift,0 DEZ 0 BCC ds_shiftdone            ; shift that buffer to pixel position
  ds_shiftloop:   LLW buffer+0 LLW buffer+2 INB buffer+2      ; set mask, keep mask
                  DEZ 0 BCS ds_shiftloop
  ds_shiftdone:     LDB buffer+2 ANR addr ORB buffer+0 SDR addr INW addr
                    LDB buffer+3 ANR addr ORB buffer+1 SDR addr LDI 63 ADW addr ; ... and move to the next line
...
; modifies: X, Y registers
; ----------------------------------------------------------------------------------
DrawSmall:      LDI <smalls SDB dptr+0 LDI >smalls SDB dptr+1 ; init sprite data pointer
                ...
                LDS 4 ANI 7 SDB shift                         ; store sub byte pixel pos
                MIZ 8,1                                       ; number of lines to process
  dm_lineloop:  LDR dptr SDB buffer+0 INW dptr CLB buffer+1   ; prepare the bit masks
                MBZ shift,0 DEZ 0 BCC dm_shiftdone            ; shift that buffer to pixel position
  dm_shiftloop:   LLW buffer+0 DEZ 0 BCS dm_shiftloop
  dm_shiftdone: LDS 5 ANI 2 CPI 2 BEQ dm_clearit
                  LDB buffer+0 ORR addr SDR addr INW addr     ; store line buffer to VRAM addr
                  LDB buffer+1 ORR addr SDR addr JPA dm_common
  dm_clearit:   LDB buffer+0 NOT ANR addr SDR addr INW addr   ; store line buffer to VRAM addr
                LDB buffer+1 NOT ANR addr SDR addr
...
; modifies: X, Y registers
; ----------------------------------------------------------------------------------
DrawWall:       LDI <wall SDB dptr+0 LDI >wall SDB dptr+1     ; hard-coded wall data pointer
                LDS 3 LL6 SDB addr+0                          ; LSB of ypos*64
                LDS 3 RL7 ANI 63 ADI >ViewPort SDB addr+1     ; MSB of ypos*64 (rotate via C)
                LDS 4 RL6 ANI 63 ADI <ViewPort OR.B addr+0    ; xpos/8
                LDS 4 ANI 7 SDB shift                         ; store sub byte pixel pos
                MIZ 16,1                                      ; number of lines to process
  dw_lineloop:  LDR dptr SDB buffer+0 INW dptr                ; prepare the bit masks
                LDR dptr SDB buffer+1 INW dptr
                LDR dptr SDB buffer+2 INW dptr CLB buffer+3
                MBZ shift,0 DEZ 0 BCC dw_shiftdone            ; shift that buffer to pixel position
  dw_shiftloop:   LLW buffer+0 RLW buffer+2 DEZ 0 BCS dw_shiftloop
  dw_shiftdone:     LDB buffer+0 ORR addr SDR addr INW addr
                    LDB buffer+1 ORR addr SDR addr INW addr
                    LDB buffer+2 ORR addr SDR addr INW addr
                    LDB buffer+3 ORR addr SDR addr LDI 61 ADW addr ; ... and move to the next line
                    DEZ 1 BNE dw_lineloop                     ; haben wir alle sprite daten verarbeitet?
                      RTS

DrawLaser:      LDB s_y LL6 SDB addr+0                        ; LSB of ypos*64
                LDB s_y RL7 ANI 63 ADI >ViewPort SDB addr+1   ; MSB of ypos*64 (rotate via C)
                LDB s_x RL6 ANI 63 ADI <ViewPort OR.B addr+0  ; xpos/8
                LDB s_x ANI 7 ADI LL0+0 SDB dl_llx            ; use sub pixel pos
                LDI 1
  dl_llx:         0xcc                                        ; trick: instruction LL0..LL7 is placed here
                SDZ 0
                LDR addr ORZ 0 SDR addr LDI 64 ADW addr
                LDR addr ORZ 0 SDR addr LDI 64 ADW addr
                LDR addr ORZ 0 SDR addr LDI 64 ADW addr
                LDR addr ORZ 0 SDR addr
                RTS

  dl_mask:      0

DeleteLaser:    LDB s_y LL6 SDB addr+0                        ; LSB of ypos*64
                LDB s_y RL7 ANI 63 ADI >ViewPort SDB addr+1   ; MSB of ypos*64 (rotate via C)
                LDB s_x RL6 ANI 63 ADI <ViewPort OR.B addr+0  ; xpos/8
                LDB s_x ANI 7 ADI LL0+0 SDB de_llx            ; use sub pixel pos
                LDI 1
  de_llx:         0xcc                                        ; trick: instruction LL0..LL7 is placed here
                NOT SDZ 0                                     ; invert mask for deletion
                LDR addr ANZ 0 SDR addr LDI 64 ADW addr
                LDR addr ANZ 0 SDR addr LDI 64 ADW addr
                LDR addr ANZ 0 SDR addr LDI 64 ADW addr
                LDR addr ANZ 0 SDR addr
                RTS
                ...
                CPI 44 BCS gdm44
                  CPI 33 BCS gdm33
                    CPI 22 BCS gdm22
                      CPI 11 BCS gdm11
                        SDS 3 LDI 0 RTS
  gdm44:        SUI 44 SDS 3 LDI 4 RTS
  gdm33:        SUI 33 SDS 3 LDI 3 RTS
  gdm22:        SUI 22 SDS 3 LDI 2 RTS
  gdm11:        SUI 11 SDS 3 LDI 1 RTS
