...
export const createFileSystem = async () => {
  ...
  await fileSystemProvider.writeFile(workspaceFileUri, textEncoder.encode(createDefaultWorkspaceContent("/Min64")), options);

  const examplesAsm = import.meta.glob<string>("../minasm/examples/*.asm", { eager: true, query: "?raw", import: "default" });
  ...
  const examplesMin = import.meta.glob<string>("../minmin/examples/*.min", { eager: true, query: "?raw", import: "default" });
  Object.entries(examplesMin).forEach(([key, value]) => {
    fileSystemProvider.writeFile(vscode.Uri.file(`/Min64/min/${key.replace("../minmin/examples/", "")}`), textEncoder.encode(value), options);
  });
